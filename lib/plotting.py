...
def plot_two_episode_stats(double_stats, smoothing_window=10, noshow=False):

    stats, stats2 = double_stats
    # Plot the episode length over time
    fig1 = plt.figure(figsize=(10,5))
    plt.plot(stats.episode_lengths)
    plt.plot(stats2.episode_lengths)
    plt.xlabel("Episode")
    plt.ylabel("Episode Length")
    plt.title("Episode Length over Time")
    if noshow:

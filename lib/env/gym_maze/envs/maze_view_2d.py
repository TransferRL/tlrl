# Original code: https://github.com/tuzzer/gym-maze/blob/master/gym_maze/envs/maze_view_2d.py
...
class MazeView2D:
    ...
    def __init__(self, maze_name="Maze2D", maze_file_path=None,
                 ...
                 has_loops=False, num_portals=0, observation_size=(3,3),
                 start_pos=None, goal_pos=None, display=False):
        ...
        if start_pos is None:
            self.__entrance = (np.random.randint(self.maze_size[0]), np.random.randint(self.maze_size[1]))
        if goal_pos is None:
            self.__goal = (np.random.randint(self.maze_size[0]), np.random.randint(self.maze_size[1]))
    ...
    def move_robot(self, dir):
        ...
        if self.__maze.is_open(self.__robot, dir):
            ...
            if self.__maze.is_portal(self.robot):
                self.__robot = np.array(self.__maze.get_portal(tuple(self.robot)).teleport(tuple(self.robot)))
            if self.display:
    ...
    def __controller_update(self):
        if self.display and not self.__game_over:
    ...
    def __draw_maze(self):
        ...
        for y in range(self.__maze.MAZE_H + 1):
            pygame.draw.line(self.maze_layer, line_colour, (0, y * self.CELL_H),
                             (self.SCREEN_W, y * self.CELL_H))

        # drawing the vertical lines
        for x in range(self.__maze.MAZE_W + 1):
            pygame.draw.line(self.maze_layer, line_colour, (x * self.CELL_W, 0),
                             (x * self.CELL_W, self.SCREEN_H))

        # breaking the walls
        for x in range(len(self.__maze.maze_cells)):
            for y in range (len(self.__maze.maze_cells[x])):
                # check the which walls are open in each cell
                walls_status = self.__maze.get_walls_status(self.__maze.maze_cells[x, y])
                dirs = ""
    ...
    def __draw_portals(self, transparency=160):
        colour_range = np.linspace(0, 255, len(self.__maze.portals), dtype=int)
        colour_i = 0
        for portal in self.__maze.portals:
    ...
    def __convert_maze(self):
        # Convert cells to matrix
        self.__maze_matrix = np.ones((self.maze_size[0] * 2 + 1, self.maze_size[1] * 2 + 1), dtype=int)
        ...
        for x in range(len(self.__maze.maze_cells)):
            for y in range(len(self.__maze.maze_cells[x])):
                ...
                _y0 = y * 2 + 1
                # print(x,y, _x,_y)
                self.__maze_matrix[_x0][_y0] = 0
                for dir, open in walls_status.items():
                    if open:
                        _x = _x0 + self.__maze.COMPASS[dir][0]
                        _y = _y0 + self.__maze.COMPASS[dir][1]
                        self.__maze_matrix[_x][_y] = 0
        # Set entrance and goal
        self.__maze_matrix[self.entrance[0]*2+1][self.entrance[1]*2+1] = 5
        self.__maze_matrix[self.goal[0]*2+1][self.goal[1]*2+1] = 9

    @property
    def maze(self):
        return self.__maze_matrix

    @property
    def current_view(self):
        x = self.__robot[0] * 2 + 1
        y = self.__robot[1] * 2 + 1
        state = np.array([self.__maze_matrix[x - 1][y], \
                          self.__maze_matrix[x + 1][y], \
                          self.__maze_matrix[x][y + 1], \
                          self.__maze_matrix[x][y - 1]])
        return state
    @property
    def robot(self):
    ...
    @property
    def CELL_W(self):
        return float(self.SCREEN_W) / float(self.__maze.MAZE_W)

    @property
    def CELL_H(self):
        return float(self.SCREEN_H) / float(self.__maze.MAZE_H)
...
class SimpleMaze2D(Maze):
    def __init__(self, maze=None, maze_size=(10, 10), start_pos=None, goal_pos=None):
        ...
        # Set start and goal position
        aisles = list(zip(*np.where(np.logical_not(self.__matrix))))
        if start_pos is None:
            i = np.random.randint(len(aisles))
            start_pos = tuple(aisles[i])
        if goal_pos is None:
            i = np.random.randint(len(aisles))
            goal_pos = tuple(aisles[i])

        # TODO: Can be changed to strings
        self.__matrix[start_pos[0]][start_pos[1]] = 5
        self.__matrix[goal_pos[0]][goal_pos[1]] = 8


    def update_matrix(self):
        # Convert cells to matrix
        self.__matrix = np.ones((self.maze_size[0] * 2 + 1, self.maze_size[1] * 2 + 1), dtype=int)
        # reconstruct the walls
        # print(self.__matrix)
        for x in range(len(self.maze_cells)):
            for y in range(len(self.maze_cells[x])):
                ...
                _y0 = y * 2 + 1
                # print(x,y, _x,_y)
                self.__matrix[_x0][_y0] = 0
                for dir, open in walls_status.items():
                    if open:
                        _x = _x0 + self.COMPASS[dir][0]
                        _y = _y0 + self.COMPASS[dir][1]
                        self.__matrix[_x][_y] = 0
    @property
